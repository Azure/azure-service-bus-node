...
export class ServiceBusMessage implements ReceivedMessage {
  ...
  async complete(): Promise<void> {
    ...
    const receiver = this._context.getReceiver(this._delivery.link.name, this.sessionId);
    if (receiver) {
  ...
  async abandon(propertiesToModify?: Dictionary<any>): Promise<void> {
    ...
    if (receiver) {
      ...
      let msg = `Cannot find the receiver with name '${this._delivery.link.name}'`;
      if (this.sessionId) msg += ` for sessionId '${this.sessionId}'.`;
      throw new Error(msg);
    }
  }
